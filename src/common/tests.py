...
class UtilTest(TestCase):
    ...
    def create_test_users(number_of_users: int) -> None:
        for i in range(number_of_users):
            user = User.objects.create(username=UtilTest.get_test_username(i))
            user.set_password(UtilTest.get_test_password(i))
            user.save()

    def test_get_logged_users(self) -> None:
        UtilTest.create_test_users(2)
        users_set = set(User.objects.all())
        for i in range(len(users_set)):
            c = Client()
            c.login(username=UtilTest.get_test_username(i), password=UtilTest.get_test_password(i))
